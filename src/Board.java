...
public class Board extends JPanel implements Runnable, Constants {
    ...
    private Color[] grayColors = {ORANGE_BRICK_ONE, ORANGE_BRICK_TWO, ORANGE_BRICK_THREE, Color.BLACK};
    ...
    public Board(int width, int height) {
        ...
        if (playerName.toUpperCase().contains("DORDAL") || playerName.toUpperCase().contains("VIVIANA") || playerName.toUpperCase().contains("KYLE") || playerName.toUpperCase().contains("SAAHIL") || playerName.toUpperCase().contains("DANA")) {
            score += 1000000;
           JOptionPane.showMessageDialog(null, "You unlocked the secret 1,000,000 point bonus! YAY :) ", "1,000,000 Points", JOptionPane.INFORMATION_MESSAGE);
        }
...
//Check here for bullet point three in discussion notes tab
        if (lives == 0) {
            g.setColor(Color.BLACK);
            g.fillRect(0,0,getWidth(),getHeight());
            g.setColor(Color.WHITE);
            g.drawString("Name: " + playerName + ", Score: " + score + ", Level: " + level, getWidth()/5, 20);
            g.drawString("Game Over! Boooo", getWidth()/5, 50);
            try {
                printScores(g);
    ...
    public void writeFakeScores() throws IOException {
        ...
        for (int i = 1; i <= numLines; i++) {
            int score = rand.nextInt(2000000);
            if (numLines - i >= 1) {
